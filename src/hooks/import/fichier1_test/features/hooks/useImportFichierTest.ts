                ...
                if (rows[i].mvt === "open") {
                    if (relations.length > 0) {
                        for (const item of relations) {
                            await upsert(id, prixParItems, type_cout_mapping.OUVERTURE, item.itemtype, item.items_id);
                        }
                    } else {
                        await upsert(id, valeur, type_cout_mapping.OUVERTURE, "Réouverture globale", null);
                    }
                }
